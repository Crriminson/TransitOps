...
import { fuelLogCreateSchema, type FuelLogCreateInput } from "@transitops/shared";
import { createFuelLog } from "../../lib/costs";
...
// Any vehicle can carry a fuel log (including IN_SHOP/RETIRED — historical
...
export default function FuelLogFormModal({ onClose }: FuelLogFormModalProps) {
  ...
  } = useForm<FuelLogCreateInput>({
    resolver: zodResolver(fuelLogCreateSchema),
    defaultValues: { date: new Date(today) },
  });

  const mutation = useMutation({
    mutationFn: (data: FuelLogCreateInput) => createFuelLog(data),
    onSuccess: () => {
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Liters" error={errors.liters?.message}>
            <input type="number" step="any" {...register("liters")} className={inputClass} />
          </Field>
